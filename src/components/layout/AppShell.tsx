...
export function AppShell({
  ...
  const startDrag = useCallback(
    (setter: (w: number) => void, min: number, getMax: () => number) =>
      (e: React.MouseEvent) => {
        e.preventDefault();
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground">
      <header className="flex items-center px-4 h-14 border-b border-border shrink-0 bg-muted/10">
      ...
      <div className="flex flex-1 overflow-hidden" ref={containerRef}>
        {/* Sidebar */}
        <aside
          className="shrink-0 border-r border-border flex flex-col overflow-y-auto bg-muted/30"
        ...
        <div
          ...
          onMouseDown={startDrag(
            setListW,
            200,
            () => getContainerWidth() - sidebarW - 300,
          )}
        />
