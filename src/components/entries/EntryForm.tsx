  ...
  return (
    ...
    >
      <div className="flex gap-2 items-center">
        <button
          type="button"
          className="cursor-grab active:cursor-grabbing text-muted-foreground shrink-0 opacity-0 group-hover/row:opacity-100 transition-opacity"
          {...attributes}
          {...listeners}
        >
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          className="shrink-0 h-8 w-8 opacity-0 group-hover/row:opacity-100 focus-within:opacity-100 transition-opacity"
...
export function EntryForm({
  ...
  const applyTemplate = (
    fields_def: Array<{ name: string; field_type: string }>,
  ) => {
    setFields((prev) => [
      ...prev,
      ...fields_def.map((f, i) => ({
      ...
      prev.map((f) =>
        f.id !== id
          ...
          : {
              ...f,
              field_value: value,
              error: validateField(f.field_type, value),
            },
      ),
    );
  const updateFieldName = (id: number, name: string) =>
    setFields((prev) =>
      prev.map((f) => (f.id === id ? { ...f, field_name: name } : f)),
    );
  const updateFieldType = (id: number, type: string) =>
    setFields((prev) =>
      prev.map((f) =>
        f.id === id ? { ...f, field_type: type, error: undefined } : f,
      ),
    );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(
        msg.includes("DuplicateTitle") || msg.includes("同分组")
          ? "同分组下已存在同名条目"
          : msg,
      );
    } finally {
        ...
        <div className="flex gap-1.5 flex-wrap">
          {QUICK_ADD.map((qa) => (
            <button
              ...
              className="text-xs bg-muted/50 text-muted-foreground hover:text-foreground hover:bg-muted rounded-md px-2 py-1 transition-colors"
            >
              {qa.label}
            </button>
          ))}
          <button
            ...
            className="text-xs border border-dashed border-border text-muted-foreground hover:text-foreground rounded-md px-2 py-1 hover:bg-accent transition-colors"
          >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {templates.map((tpl) => {
            ...
            try {
              tplFields = JSON.parse(tpl.fields);
            } catch {
              /* ignore */
            }
            return (
              <button
                key={tpl.id}
                type="button"
                onClick={() => applyTemplate(tplFields)}
