...
export function EntryCard({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "w-full text-left rounded-lg transition-all flex items-stretch group border border-transparent",
        selected
            ...
            <div className="flex gap-1 mt-1 flex-wrap">
              {tags.slice(0, 3).map((tag) => (
                <Badge
                  key={tag}
                  variant="secondary"
                  className="text-[10px] px-1.5 py-0 bg-muted/60 text-muted-foreground font-normal hover:bg-muted"
                >
                  {tag}
                </Badge>
              ))}
